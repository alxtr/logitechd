...
`)
	...
	if value.SmartShift == nil || value.SmartShift.Enabled == nil || !*value.SmartShift.Enabled || value.SmartShift.Threshold == nil || *value.SmartShift.Threshold != 100 || value.SmartShift.Torque == nil || *value.SmartShift.Torque != 100 {
...
func TestLoadRejectsUnknownFieldsAndMultipleDocuments(t *testing.T) {
	for name, data := range map[string]string{
		"root":        "unknown: true\n",
		"scroll mode": "scroll_mode: ratchet\n",
		"nested":      "smart_shift:\n  threshold: 10\n  unknown: true\n",
		"action":      "buttons:\n  0x0053:\n    action: key\n    unknown: A\n",
		"multiple":    "{}\n---\n{}\n",
	} {
...
func TestLoadValidatesSmartShiftEnabledCombinations(t *testing.T) {
	...
	if _, err := Load([]byte("smart_shift:\n  enabled: false\n  threshold: 100\n")); err == nil || !errors.Is(err, ErrInvalidConfig) {
		t.Fatalf("disabled SmartShift with threshold error = %v, want invalid config", err)
	}
}
